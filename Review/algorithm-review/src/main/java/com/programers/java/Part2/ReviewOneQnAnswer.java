...
public class ReviewOneQnAnswer {
    ...
    public int solution3(int[] nums) {
        ...
        return Math.min(list.size(), type);
    }
    ...
    public int solution1(int[] nums) {
        ...
        // 1. 몇개의 포켓몬을 골라야 하는지 알기
        int type = nums.length / 2;


        // 2. map에 현재 배열에 있는 각 종류의 포켓몬을 넣기.
        for(int n : nums) {
            if(map.get(n) == null) {
                map.put(n, 1);
                continue;
        ...
        // 3. type과 비교하며 총 몇개의 포켓몬을 넣어야 하는지 알기
        if(map.size() >= type) return type;
